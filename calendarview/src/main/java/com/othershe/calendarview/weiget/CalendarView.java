...
public class CalendarView extends ViewPager {
    ...
    private List<int[]> disableDate = new ArrayList<>(); // 禁用日期
    private Map<String, String> specifyMap = new HashMap<>();// 已满日期
    ...
    private void refreshMonthView(int position) {
        ...
        if (mAttrsBean.getChooseType() == 1) {//多选
            ...
            monthView.refresh(lastClickDate[1], flag, disableDate, specifyMap, nowDate);
        }
    }
    ...
    /**
     * 将指定日期的农历替换成对应文字 修改完刷新
     *
     * @param year  当前年
     * @param month 当前月
     */
    public CalendarView setSpecifyMapAndRefresh(HashMap<String, String> map, int year, int month) {
...
//        mAttrsBean.setSpecifyMap(map);
     ...
     * 设置日历禁用范围
     *
     * @param startDate1 禁用startDate之前的日期
     * @param endDate    禁用endDate之后的日期
     * @param year       当前年
     * @param month      当前月
     * @return
     */
    public CalendarView setDisableStartEndDateAndRefresh(String startDate1, String endDate, int year, int month) {
        mAttrsBean.setDisableStartDate(CalendarUtil.strToArray(startDate1));
        mAttrsBean.setDisableEndDate(CalendarUtil.strToArray(endDate));
        int destPosition = CalendarUtil.dateToPosition(year, month, startDate[0], startDate[1]);
        if (destPosition == currentPosition) {
            refreshMonthView(destPosition);
     ...
     * 设置日历禁用日期
     */
    public CalendarView setDisableDate(List<String> dates) {
        List<int[]> multiDates = new ArrayList<>();
        for (String date : dates) {
            int[] d = CalendarUtil.strToArray(date);
            if (isIllegal(d)) {
        ...
        mAttrsBean.setDisableDate(multiDates);
        return this;
    }
     ...
     * @param month 当前月
     */
    public CalendarView setDisableDateAndRefresh(List<String> dates, int year, int month) {
        List<int[]> multiDates = new ArrayList<>();
        for (String date : dates) {
            int[] d = CalendarUtil.strToArray(date);
            if (isIllegal(d)) {
