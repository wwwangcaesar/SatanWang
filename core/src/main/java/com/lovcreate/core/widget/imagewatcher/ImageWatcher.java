...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    private int mErrorImageRes = R.drawable.error_picture;
     ...
     * @param imageGroups 被点击的ImageView的所在列表，加载图片时会提前展示列表中已经下载完成的thumb图片
     * @param urlList     被加载的图片url列表，数量必须大于等于 imageGroupList.size。 且顺序应当和imageGroupList保持一致
     */
    public void show(ImageView i, List<ImageView> imageGroups, final List<String> urlList) {
        if (loader == null) {
            throw new NullPointerException("please invoke `setLoader` first [loader == null]");
        }
        List<ImageView> imageGroupLists = new ArrayList<>();
        List<ImageView> imageGroupList = new ArrayList<>();
        if (imageGroups.get(0).getTag() instanceof String) {
            for (ImageView view : imageGroups) {
                for (String url : urlList) {
                    if (null != view.getTag() && view.getTag().equals(url)) {
                        if (imageGroupLists.size() > 0) {
                            boolean isHas = false;
                            for (ImageView im : imageGroupLists) {
                                if (im.getTag().equals(url)) {
                                    isHas = true;
                                }
                            }
                            if (!isHas) {
                                imageGroupLists.add(view);
                            }
                        } else {
                            imageGroupLists.add(view);
                        }
            ...
            for (int j = 0; j < imageGroupLists.size(); j++) {
                for (int k = 0; k < urlList.size(); k++) {
                    if (imageGroupLists.get(j).getTag().equals(urlList.get(k))) {
                        imageGroupList.add(imageGroupLists.get(k));
                    }
                }
            }
        } else {
            imageGroupList.addAll(imageGroups);
        }
